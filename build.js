#!/usr/bin/env node
// Injects sitewide links (nav + footer) across all pages without hand-editing each file.
...
// This script does a targeted insertion instead: <a> tags at fixed anchor points.
...
const LINKS = {
  ro: [
    { href: '/test-anxietate/', label: 'Test anxietate' },
    { href: '/wellbeing/', label: 'Wellbeing' },
  ],
  en: [
    { href: '/en/anxiety-test/', label: 'Anxiety Test' },
  ],
};
...
function injectNav(html, relPath) {
  const links = LINKS[localeOf(relPath)] || [];
  ...
  for (const { href, label } of links) {
    ...
    block = block.replace('<span class="lang-sw">', linkTag + '<span class="lang-sw">');
    insertedAny = true;
  }
  return { html: html.replace(m[0], block), status: insertedAny ? 'inserted' : 'already' };
}
...
function injectFooter(html, relPath) {
  ...
  for (const { href, label } of links) {
    ...
    if (block.includes('class="flinks"')) {
      block = block.replace(/(<div class="flinks">[\s\S]*?)(<\/div>)/, `$1  ${linkTag}\n  $2`);
    } else if (/<\/p>\s*<\/footer>/.test(block)) {
      block = block.replace(/<\/p>\s*<\/footer>/, ` · ${linkTag}</p>\n</footer>`);
    } else {
      block = block.replace(/<\/footer>/, ` · ${linkTag}\n</footer>`);
    }
    insertedAny = true;
  }
  return { html: html.replace(m[0], block), status: insertedAny ? 'inserted' : 'already' };
...
function main() {
  ...
  console.log(`Nav skipped (minimal blog/back nav, by design): ${stats.navSkippedMinimal}`);
